...
datasets = ['./data/train.pkl', r'./data/train_caption.txt']
valid_datasets = ['./data/2014.pkl', './data/2014_caption.txt']
valid_output = ['./log/valid_decode_result.txt']
valid_result = ['./log/valid.wer']
saveto = r'./log/WAP_params.pkl'
...
for eidx in range(max_epochs):
    ...
    for x, y in train:
        ...
        # forward
        scores, alphas, sc_ctx = WAP_model(params, x, x_mask, y, y_mask)
        sc_ctx = sc_ctx.squeeze(2).squeeze(2)

        # TODO for optim
        tc_ctx = torch.zeros((sc_ctx.shape[0], 111)).cuda()
        with torch.no_grad():
            for i in range(sc_ctx.shape[0]):
                ls = y[i].cpu().numpy().tolist()
                for j in ls:
                    tc_ctx[i][j] = 1

        # sc_ctx_prob.contiguous()
        # sc_ctx_prob = sc_ctx_prob.view(-1, sc_ctx_prob.shape[1])
        ...
        # loss_bce_ctx = criterion_BCE(sc_ctx, tc_ctx)
        loss_focal_ctx = criterion_focal(sc_ctx, tc_ctx)

        loss = criterion(scores, y.view(-1))
        loss = loss.view(y.shape[0], y.shape[1])
        loss = (loss * y_mask).sum(0) / y_mask.sum(0)
        loss = loss.mean() + 0.5 * loss_focal_ctx.mean()
        # loss = loss.mean()
        loss_s += loss.item()
